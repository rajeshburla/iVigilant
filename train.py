# -*- coding: utf-8 -*-
"""
Created on Wed Mar 27 18:46:26 2019
...
import preprocessing as process_data_files
import utils
...
def load_datasets_development(preset_key):
    ...
    PRESETS_DICTIONARY = {'PAN18_English': {'dataset_name': 'PAN 2018 English',
                                            'xmls_directory': 'U:/Desktop/textAnalytics/New folder/pan19-author-profiling-training-2019-02-18-20190302T192830Z-001/pan19-author-profiling-training-2019-02-18/en/',
                                            'truth_path': 'U:/Desktop/textAnalytics/New folder/pan19-author-profiling-training-2019-02-18-20190302T192830Z-001/pan19-author-profiling-training-2019-02-18/en-truth/truth.txt',
                                            'txts_destination_directory': 'U:/Desktop/textAnalytics/New folder/pan19-author-profiling-training-2019-02-18-20190302T192830Z-001/pan19-author-profiling-training-2019-02-18/txtFiles',
                                            }
                          }
    PRESET = PRESETS_DICTIONARY[preset_key]

    # Load the PAN 2018 training dataset and the truth from the files into lists
    print("Loading the %s training dataset and the truth...", PRESET['dataset_name'])
...
def load_datasets_tira_evaluation(test_dataset_main_directory, preset_key):
    ...
    print("Loading the %s training dataset and truth...", PRESET['dataset_name'])
...
def extract_features(docs_train, docs_test, preset_key):
    ...
    print("Training set word & character ngrams .shape = %s", X_train_ngrams_tfidf.shape)
    ...
    print("Test set word & character ngrams .shape = %s", X_test_ngrams_tfidf.shape)
...
def extract_features_offensive_words(docs_train, docs_test):
    ...
    *flame_expressions_dict*
    ...
    print("@ %.2f seconds: Finished dimensionality reduction (LSA) in *extract_features_offensive_words()* on "
...
def count_offensive_words(docs, pickle_path_pattern=None):
    ...
    if (pickle_path is not None) and os.path.isfile(pickle_path):
        ...
        print('Function bypassed: The counts of offensive words was loaded from pickle "%s" instead.', pickle_path)
    ...
    *flame_expressions_dict*
    ...
    print("@ %.2f seconds: Finished preprocessing the tweets in *count_offensive_words()*",
                time.process_time())
    ...
    print("@ %.2f seconds: Finished compiling the regex patterns into regex objects.",
                time.process_time())
    ...
    print("@ %.2f seconds: Finished counting the occurrences of offensive words", time.process_time())
    ...
    if pickle_path is not None:
        ...
        print('The counts of offensive words was pickled to: "%s"', pickle_path)
...
def train_and_test_model(clf, X_train, y_train, X_test, y_test):
    ...
    print("@ %.2f seconds: Finished training the model and predicting class labels for the test set",
                time.process_time())

    # Simple evaluation using numpy.mean
    print("np.mean %%Accuracy: %f", np.mean(y_predicted == y_test) * 100)
...
def train_model_and_predict(clf, X_train, y_train, X_test, author_ids_test, preset_key,
    ...
    if write_to_xml_files:
        print("Writing the predictions to XML files.")
...
def main_development():
    ...
    for presetKey in ("PAN18_English",
        ...
        print("Running main_development() for preset: %s", presetKey)
...
def main_tira_evaluation():
    ...
    print("test_dataset_main_directory = %s", test_dataset_main_directory)
    print("prediction_xmls_destination_main_directory = %s\n", prediction_xmls_destination_main_directory)

    for preset_key in ("PAN18_English", "PAN18_Spanish", "PAN18_Arabic"):
        print("Running main_tira_evaluation() for preset: %s", preset_key)
